...
@click.option("--username", help="Username for user related operations", default=None)
@click.option(
    "--password",
    help="Password for user related operations - supplying this via argument is unsafe",
    default=None,
)
@click.pass_context
...
def _create_user(ctx):
    ...
    username, passhash = _get_credentials(
        ctx.obj["username"], ctx.obj["password"], ctx.obj["db"]
    )
    ...
    new_user = ctx.obj["db"].objectmodels["user"](
        {"uuid": str(uuid4()), "created": std_now()}
    )
...
@click.option(
    "--yes", "-y", help="Do not ask for confirmation", default=False, is_flag=True
)
@click.pass_context
...
def change_password(ctx):
    ...
    change_user = ctx.obj["db"].objectmodels["user"].find_one({"name": username})
...
@click.option(
    "--search", help="Specify a term for searching", default=None, metavar="<text>"
)
@click.option("--uuid", help="Print user" "s uuid as well", default=False, is_flag=True)
@click.option(
    "--active",
    help="Print user" "s account activation status",
    default=False,
    is_flag=True,
)
@click.pass_context
...
def disable(ctx):
    ...
    change_user = (
        ctx.obj["db"].objectmodels["user"].find_one({"name": ctx.obj["username"]})
    )
